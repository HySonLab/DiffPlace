...
print('##### MODEL #####')
for dataset_name in train_dataset_names:
    print(f"Attempting to load data for {dataset_name}")
    for i in range(0, args.itermax):
        file_path = f'data/{dataset_name}/iter_{i}_node_label_full_{args.hashcode}_.npy'
        ...
        if os.path.isfile(file_path):
            ...
            if list_tuple_graph:
                list_tuple_graph = fit_topo_geom(list_tuple_graph)
                train_list_tuple_graph.extend(list_tuple_graph)
                print(f"Loaded {len(list_tuple_graph)} graphs for {dataset_name}, iter {i}")
            else:
                print(f"Warning: No data loaded for {dataset_name}, iter {i}")
        else:
            print(f"File not found: {file_path}")

print(f"Total graphs loaded: {len(train_list_tuple_graph)}")

if not train_list_tuple_graph:
    raise ValueError("train_list_tuple_graph is empty. No data to process.")

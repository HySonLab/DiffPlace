import engine.networks.pos_encoding as pos_encoding
import torch
import torch.nn as nn
import torch.nn.functional as F
import torch.distributions as tfd
import numpy as np
import engine.networks as networks
from omegaconf import open_dict
from engine import orientations
from engine import guidance
import os
from engine import schedulers
...
class LegacyDiffusionModel(MixedDiffusionModel):
    """
...
class DiffPlaceModel(nn.Module):
    """
    DiffPlace: Conditional Diffusion Model for VLSI Placement
    
    Supports two backbone types:
    - 'scalar': AttGNN (original legacy backbone)
    - 'vector': VectorGNN (new vector-wise message passing backbone)
    
    Key differences for VectorGNN:
    - Explicit relative position encoding in message passing
    - FiLM conditioning at every layer
    - Zero-init output for stable training
    - Processes (pos, feat) separately where pos = x[:, :, :2]
    
    Rotation Support:
    - Input/Output can be 2D (x, y) or 3D (x, y, rotation_logit)
    - Message passing always uses only 2D positions (x, y) for relative encoding
    - Rotation channel (if present) is predicted at output but not used in message passing
    - Set input_shape=(V, 3) to enable rotation output
    """
    ...
    backbones = {
        ...
        "graph_transformer": networks.GraphTransformer,
        "vector_gnn": networks.VectorGNN,
        "vector_gnn_large": networks.VectorGNNLarge,
        "vector_gnn_small": networks.VectorGNNSmall,
    }
    ...
    def __init__(
            ...
            backbone_type="scalar",  # 'scalar' | 'vector'
            # Decoupled Guidance parameters
            energy_guidance_weight=1.0,
            energy_grad_steps=5,
            energy_lr=0.1,
            legality_guidance_weight=1.0,
            legality_grad_steps=10,
            legality_lr=0.05,
            legality_softmax_min=5.0,
            legality_softmax_max=50.0,
            guidance_start_step=0.8,  # Start guidance at t < 0.8*T
            **kwargs,
    ):
        super().__init__()
        self.device = device
        ...
        # Decoupled Guidance config
        self.energy_guidance_weight = energy_guidance_weight
        self.energy_grad_steps = energy_grad_steps
        self.energy_lr = energy_lr
        self.legality_guidance_weight = legality_guidance_weight
        self.legality_grad_steps = legality_grad_steps
        self.legality_lr = legality_lr
        self.legality_softmax_min = legality_softmax_min
        self.legality_softmax_max = legality_softmax_max
        self.guidance_start_step = guidance_start_step

        # Select backbone based on backbone_type
        if backbone_type == "vector":
            # Use VectorGNN for vector-wise message passing
            if backbone not in ["vector_gnn", "vector_gnn_large", "vector_gnn_small"]:
                backbone = "vector_gnn"
                print(f"[DiffPlaceModel] backbone_type='vector', using VectorGNN backbone")
            
            # Determine input/output dimensions
            # input_shape[1] can be 2 (x, y) or 3 (x, y, rotation)
            pos_dim = input_shape[1]
            if pos_dim == 3:
                print(f"[DiffPlaceModel] Rotation support enabled: output will be (B, V, 3) with rotation logit")
            
            with open_dict(backbone_params):
                backbone_params.update({
                    "in_node_features": pos_dim,  # 2 for (x,y) or 3 for (x,y,rotation)
                    "out_node_features": pos_dim,  # Output matches input dimension
                    "t_encoding_dim": t_encoding_dim,
                    "device": device,
                    "mask_key": mask_key if use_mask_as_input else None,
                })
        else:
            # Use AttGNN or other scalar backbones
            with open_dict(backbone_params):
                backbone_params.update({
                    "in_node_features": input_shape[1],
                    "out_node_features": input_shape[1],
        ...
        self.net = DiffPlaceModel.backbones[backbone](**backbone_params)
        self.backbone_name = backbone

        self.time_embed = SinusoidalPositionalEmbedding(t_encoding_dim, max_diffusion_steps)

        from schedulers_ddpm import DDPMScheduler
        self.scheduler = DDPMScheduler(
            num_train_timesteps=max_diffusion_steps,
        ...
        self.scheduler.alphas_cumprod_prev = self.scheduler.alphas_cumprod_prev.to(device)
        ...
        self.scheduler.posterior_variance = self.scheduler.posterior_variance.to(device)
        self.scheduler.posterior_log_variance_clipped = self.scheduler.posterior_log_variance_clipped.to(device)
        self.scheduler.posterior_mean_coef1 = self.scheduler.posterior_mean_coef1.to(device)
        self.scheduler.posterior_mean_coef2 = self.scheduler.posterior_mean_coef2.to(device)

        self._lossfn = nn.MSELoss(reduction="mean")
        self._epsilon_dist = tfd.Normal(
    ...
    def forward(self, x, cond, t):
        ...
        # t: (B,) integer timesteps
        t_embed = self.time_embed(t)  # (B, t_encoding_dim)
        noise_pred = self.net(x, cond, t_embed)  # (B, V, F)
        return noise_pred.view(*x.shape)

    def loss(self, x, cond, t=None):
        # x: (B, V, F) clean data
        # cond: PyG Data object
        # t: (B,) integer timesteps (optional, will sample if None)
        B = x.shape[0]

        if t is None:
            t = self.scheduler.sample_timesteps(B, device=x.device)

        mask = self.get_mask(x, cond)

        noise = self._epsilon_dist.sample(x.shape).squeeze(dim=-1)
        x_noisy = self.scheduler.add_noise(x, noise, t)

        if mask is not None:
            x_noisy = torch.where(mask, x, x_noisy)

        noise_pred = self.forward(x_noisy, cond, t)

        loss = self._compute_loss(noise_pred, noise, mask)

        pred_masked = noise_pred.detach()
        if mask is not None:
            pred_masked = pred_masked[torch.logical_not(mask).expand(noise_pred.shape)]
        
        metrics = {
            "epsilon_theta_mean": pred_masked.mean().cpu().numpy(),
            "epsilon_theta_std": pred_masked.std().cpu().numpy(),
        }
        return loss, metrics

    def _compute_loss(self, prediction, target, mask=None):
        if mask is not None:
            numel = torch.numel(mask)
            squared_error = torch.square(prediction - target)
            squared_error = squared_error.masked_fill(mask, 0)
            mse = torch.mean(squared_error) * (numel / (numel - torch.sum(mask)))
            return mse
        else:
            return self._lossfn(prediction, target)
    ...
    def get_mask(self, x, cond):
        """
        Get mask for fixed/immovable nodes.
        
        Combines masks from:
        - mask_key (e.g., 'is_ports') if specified
        - 'is_fixed' attribute if present (for ISPD benchmarks)
        
        Returns:
            mask: (1, V, 1) boolean tensor where True = node is fixed
        """
        B, V, F = x.shape
        mask = None
        
        # Check primary mask key (e.g., is_ports)
        if self.mask_key and self.mask_key in cond:
            mask = cond[self.mask_key].bool()
        
        # Also check for is_fixed (ISPD benchmarks)
        if hasattr(cond, 'is_fixed') and cond.is_fixed is not None:
            is_fixed = cond.is_fixed.bool()
            if mask is not None:
                mask = mask | is_fixed  # Combine masks
            else:
                mask = is_fixed
    ...
    @torch.no_grad()
    def sample(self, B, cond, x_init=None, num_inference_steps=None, intermediate_every=0, 
               mask_override=None, fixed_positions=None):
        """
        Standard DDPM sampling without guidance.
        
        Args:
            B: batch size
            cond: PyG Data object
            x_init: (B, V, F) optional initial positions (for ports)
            num_inference_steps: number of denoising steps
            intermediate_every: save intermediate every N steps
            mask_override: Override mask for fixed nodes
            fixed_positions: Ground truth positions for fixed nodes
        """
        if num_inference_steps is None:
            num_inference_steps = self.max_diffusion_steps

        V = cond.x.shape[0]
        F = self.input_shape[1]
        batch_shape = (B, V, F)

        x = self._epsilon_dist.sample(batch_shape).squeeze(dim=-1).to(self.device)

        # Compute mask for fixed nodes
        mask = None
        if mask_override is not None:
            mask = mask_override.view(1, V, 1)
        elif x_init is not None:
            mask = self.get_mask(x_init, cond)
        else:
            dummy_x = torch.zeros(B, V, F, device=self.device)
            mask = self.get_mask(dummy_x, cond)

        # Determine fixed positions
        init_positions = fixed_positions if fixed_positions is not None else x_init

        # Initialize fixed nodes to ground truth (no noise!)
        if mask is not None and init_positions is not None:
            x = torch.where(mask, init_positions, x)
        ...
        for i, t in enumerate(timesteps):
            t_batch = t.expand(B)
            noise_pred = self.forward(x, cond, t_batch)

            x, pred_x0 = self.scheduler.step(noise_pred, t.item(), x)

            # Keep fixed nodes at ground truth positions
            if mask is not None and init_positions is not None:
                x = torch.where(mask, init_positions, x)

            if intermediate_every > 0 and (i + 1) % intermediate_every == 0:
    ...
    def sample_with_guidance(
        ...
        """
        Sampling with custom guidance function.
        
        Args:
            B: batch size
            guidance_fn: function(x_hat) -> guidance_delta
            fixed_positions: Ground truth positions for fixed nodes
        """
        if num_inference_steps is None:
            num_inference_steps = self.max_diffusion_steps
        ...
        # Initialize fixed nodes to ground truth
        if mask is not None and init_positions is not None:
            x = torch.where(mask, init_positions, x)
        ...
        for i, t in enumerate(timesteps):
            ...
            x, pred_x0 = self.scheduler.step_with_guidance(
                noise_pred, t.item(), x, guidance_fn=guidance_fn
            )
    ...
    def forward_diffusion(self, x, cond, intermediate_every=0):
        # x: (B, V, F) clean data
        mask = self.get_mask(x, cond)
        intermediates = [x.clone()]

        for t in range(0, self.max_diffusion_steps, max(1, intermediate_every)):
            ...
            intermediates.append(x_noisy.clone())

        return intermediates

    def reverse_samples_diffplace(
            self,
            B,
            cond,
            x_init=None,
            ...
            intermediate_every=0,
            mask_override=None,
            enable_energy_guidance=True,
            enable_legality_guidance=True,
            return_metrics=False,
            fixed_positions=None,
    ):
        """
        DiffPlace sampling with Decoupled Guidance:
        - Step 1: Standard DDPM denoising
        - Step 2a: Energy Gradient (Global HPWL optimization)
        - Step 2b: Manifold Projection (Local legality enforcement)
        
        Args:
            B: Batch size
            cond: PyG Data with graph structure
            x_init: Initial positions for masked nodes (usually ports)
            num_inference_steps: Number of denoising steps
            intermediate_every: Save intermediate every N steps
            mask_override: Override mask (1, V, 1) for fixed nodes
            enable_energy_guidance: Enable HPWL optimization
            enable_legality_guidance: Enable overlap removal
            return_metrics: Return guidance metrics
            fixed_positions: Ground truth positions for fixed nodes (B, V, F)
                           If provided, fixed nodes are reset to these positions at each step
        
        Fixed Node Inpainting:
            For ISPD benchmarks, terminals/fixed nodes should never move.
            - If `is_fixed` mask exists in cond, those nodes are kept at ground truth
            - At each timestep t, fixed nodes are reset: x[fixed] = fixed_positions[fixed]
            - This is different from noisy inpainting (no noise added to fixed nodes)
        """
        if num_inference_steps is None:
            num_inference_steps = self.max_diffusion_steps
        ...
        if mask_override is not None:
            ...
            # Try to get mask from cond even without x_init
            dummy_x = torch.zeros(B, V, F, device=self.device)
            mask = self.get_mask(dummy_x, cond)

        # Determine fixed positions to use
        init_positions = fixed_positions if fixed_positions is not None else x_init
        
        # Initialize fixed nodes to their ground truth positions (no noise!)
        if mask is not None and init_positions is not None:
            x = torch.where(mask, init_positions, x)
        ...
        guidance_start_t = int(self.guidance_start_step * self.max_diffusion_steps)

        for i, t in enumerate(timesteps):
            t_int = t.item()
            t_batch = t.expand(B)

            # ========== Step 1: DDPM Denoising ==========
            with torch.no_grad():
                noise_pred = self.forward(x, cond, t_batch)

            # Predict x0 from noise prediction
            x0_hat = (
                x - self.scheduler.sqrt_one_minus_alphas_cumprod[t_int] * noise_pred
            ) / self.scheduler.sqrt_alphas_cumprod[t_int]

            # ========== Step 2: Decoupled Guidance ==========
            step_metrics = {"t": t_int}

            if t_int < guidance_start_t:
                # Step 2a: Energy Gradient (Global HPWL)
                if enable_energy_guidance and self.energy_guidance_weight > 0:
                    x0_hat, energy_delta = self.apply_energy_gradient(
                        x0_hat, cond, mask
                    )
                    step_metrics["energy_delta"] = energy_delta

                # Step 2b: Manifold Projection (Local Legality)
                if enable_legality_guidance and self.legality_guidance_weight > 0:
                    x0_hat, legality_delta = self.apply_manifold_projection(
                        x0_hat, cond, t_int, mask
                    )
                    step_metrics["legality_delta"] = legality_delta

            # ========== Step 3: Compute x_{t-1} from guided x0_hat ==========
            # Re-compute posterior mean with guided x0
            posterior_mean = (
                self.scheduler.posterior_mean_coef1[t_int] * x0_hat
                + self.scheduler.posterior_mean_coef2[t_int] * x
            )

            # Add noise for t > 0
            if t_int > 0:
                noise = self._epsilon_dist.sample(batch_shape).squeeze(dim=-1).to(self.device)
                posterior_std = torch.sqrt(self.scheduler.posterior_variance[t_int])
                x = posterior_mean + posterior_std * noise
            else:
                x = posterior_mean

            # Apply mask to fix port/terminal positions (Inpainting)
            # Fixed nodes are reset to ground truth at EVERY timestep
            if mask is not None and init_positions is not None:
                x = torch.where(mask, init_positions, x)
        ...
        if return_metrics:
            return x, intermediates, metrics_history
        return x, intermediates

    @torch.enable_grad()
    def apply_energy_gradient(self, x0_hat, cond, mask=None):
        """
        Step 2a: Global HPWL optimization via gradient descent.
        Optimizes placement to minimize total wirelength.
        
        Input:
            x0_hat: (B, V, 2) predicted clean placement
            cond: PyG Data object
            mask: (1, V, 1) boolean mask for fixed positions
        
        Returns:
            x0_guided: (B, V, 2) placement after HPWL optimization
            delta_norm: float, magnitude of update
        """
        x0_current = x0_hat.detach().clone().requires_grad_(True)
        ...
        initial_pos = x0_current.detach().clone()

        for _ in range(self.energy_grad_steps):
            optimizer.zero_grad()

            # Compute HPWL energy
            energy = self.energy_guidance_weight * guidance.hpwl_guidance_potential(
                x0_current, cond
            )
            energy_sum = energy.sum()
            energy_sum.backward()

            # Zero out gradients for masked (fixed) positions
            if mask is not None:
                x0_current.grad = x0_current.grad * (~mask).float()

            optimizer.step()

        delta = x0_current.detach() - initial_pos
        delta_norm = delta.norm().item()

        return x0_current.detach(), delta_norm

    @torch.enable_grad()
    def apply_manifold_projection(self, x0_hat, cond, t, mask=None):
        """
        Step 2b: Local legality constraint enforcement via projection.
        Projects placement onto legal manifold (no overlaps, within bounds).
        
        Softmax factor increases as t decreases (stricter constraints near t=0).
        
        Input:
            x0_hat: (B, V, 2) predicted placement
            cond: PyG Data object
            t: int, current timestep
            mask: (1, V, 1) boolean mask for fixed positions
        
        Returns:
            x0_projected: (B, V, 2) placement after legality projection
            delta_norm: float, magnitude of update
        """
        x0_current = x0_hat.detach().clone().requires_grad_(True)

        # Compute softmax factor: increases as t → 0 for stricter enforcement
        t_ratio = t / self.max_diffusion_steps
        softmax_factor = self.legality_softmax_max - t_ratio * (
            self.legality_softmax_max - self.legality_softmax_min
        )

        initial_pos = x0_current.detach().clone()

        for step in range(self.legality_grad_steps):
            # Compute legality potential (overlap + boundary)
            legality_potential = guidance.legality_guidance_potential(
                x0_current, cond, softmax_factor=softmax_factor, mask=mask
            )

            # Compute gradient
            grad = torch.autograd.grad(
                legality_potential.sum(),
                x0_current,
                create_graph=False,
                retain_graph=False
            )[0]
            ...
            # Gradient descent step (move towards legal manifold)
            with torch.no_grad():
                x0_current = x0_current - self.legality_lr * grad
    ...
    def get_legality_softmax_factor(self, t):
        """Compute softmax factor for legality potential based on timestep."""
        t_ratio = t / self.max_diffusion_steps
        if t_ratio > self.guidance_start_step:
            return self.legality_softmax_min
        factor = self.legality_softmax_max - t_ratio * (
            self.legality_softmax_max - self.legality_softmax_min
        )
        return factor

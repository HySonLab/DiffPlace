...
#     return hop1
def fo_average(g):
    g.ndata['addnlfeat'] = g.ndata['feat']
    g.ndata['wts'] = torch.ones(g.number_of_nodes(), 1)
    g.update_all(message_func=dgl.function.copy_u(u='addnlfeat', out='inter'),
                 reduce_func=dgl.function.sum(msg='inter', out='wtmsg'))
    g.update_all(message_func=dgl.function.copy_u(u='wts', out='inter'),
                 reduce_func=dgl.function.sum(msg='inter', out='wts_sum'))
    return g.ndata['wtmsg'] / (g.ndata['wts_sum'] + 1e-6)
...
def get_pin_density(shape, bin_x, bin_y, xdata, ydata, edge) -> np.ndarray:
    pin_density = np.zeros(shape, dtype=float)
...
def get_node_density(shape, bin_x, bin_y, xdata, ydata) -> np.ndarray:
    node_density = np.zeros(shape, dtype = float)
...
def load_data(dir_name: str, given_iter, index: int, hashcode: str,
    ...
    node_pos_code = np.zeros([n_node, n_dim], dtype=float)
